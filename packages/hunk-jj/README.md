...
Private, statically bundled Jujutsu provider for Hunk. This `0.0.0` workspace is not a public or
versioned SDK.

The package exports its provider from `src/index.ts`. It supports working-copy and revision reviews,
history, exact source reads where the revision shape permits them, and signature-based refresh.
Staged and stash reviews are not supported.
...
provider source. Run `bun test packages/hunk-jj` and `bun run deps:check` after changes.
